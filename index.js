...
async function loadAllImages(imageData) {
  ...
  for (let i = 0; i < imageData.length; i++) {
    try {
      ...
      showStatus(`Loading images... ${i + 1}/${imageData.length}`, "info");
    } catch (error) {
...
function combineImages(images) {
  ...
  const maxBottom = Math.max(...images.map((img) => img.top + img.height));
...
function validateAndMergeBlocks(canvas, blocks) {
  ...
  while (i < blocks.length) {
    ...
    while (currentBlock.width < minAcceptableWidth && i < blocks.length - 1) {
      i++;
  ...
  for (let i = 0; i < validatedBlocks.length; i++) {
    ...
    if (currentBlock.width < veryNarrowThreshold && mergedBlocks.length > 0) {
